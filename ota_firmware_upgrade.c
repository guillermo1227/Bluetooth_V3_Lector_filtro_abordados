...
void                            event_select_SRT(void);
void                            event_select_CHF(void);

extern void gap_stack_init(void);
extern void spp_stack_init(void);
...
wiced_timer_t timer_stack;
void f_stack(uint32_t data);
//--------------------------------------------------------
...
//    wiced_set_debug_uart(WICED_ROUTE_DEBUG_TO_WICED_UART);
	...
	numbytes10 = wiced_hal_read_nvram( WICED_NVRAM_VSID_START+10, sizeof(flag10), &flag10, &status10 );
	WICED_BT_TRACE("FLAG10:%d\n", flag10);
	 if(flag10==2)
	 {
	   WICED_BT_TRACE("ENTRO3%d\n");
	   wiced_init_timer(&timer_stack, f_stack,0 ,WICED_SECONDS_TIMER);
	   wiced_start_timer(&timer_stack, clock_change_stack);
	   wiced_hal_gpio_configure_pin( LED_GPIO_3, GPIO_OUTPUT_ENABLE, GPIO_PIN_OUTPUT_LOW );
       wiced_bt_stack_init(app_management_callback, &app_cfg_settings, app_buf_pools);//CSS
	 }
	 else if (flag10==1)
	 {
		 WICED_BT_TRACE("ENTRO2%d\n");
		 //spp_stack_init();
	 }
	 else
	 {
...
void f_stack(uint32_t data)
{
	...
	{
		flag10 = 0;
		numbytes10 = wiced_hal_write_nvram( WICED_NVRAM_VSID_START+10, sizeof(flag10), &flag10, &status10 );
...
void event_select_CHF(void)
{
	WICED_BT_TRACE("Change stack spp\n");
	flag10 = 0;
	numbytes10 = wiced_hal_write_nvram( WICED_NVRAM_VSID_START+10, sizeof(flag10), &flag10, &status10 );
}
